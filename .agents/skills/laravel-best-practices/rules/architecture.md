...
Depend on contracts at system boundaries (notification channels, search providers, external APIs) for testability and swappability.

Incorrect (concrete dependency):
```php
class SearchService
{
    public function __construct(private AlgoliaSearchClient $client) {}
}
```
...
{
    public function index(Document $document): SearchIndexResult;
}
...
```php
$this->app->bind(SearchClient::class, AlgoliaSearchClient::class);
```
